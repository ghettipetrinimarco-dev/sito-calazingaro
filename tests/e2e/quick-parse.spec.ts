...
test("quick parse playground renders a complete draft", async ({ page }) => {
  ...
  await page.getByLabel("Frase ricevuta").fill("franco domani tavolo 7 3 21")
  await page.getByRole("button", { name: "Interpreta frase" }).click()

  const draft = page.locator("section").filter({ hasText: "Bozza generata" })
  ...
  await expect(draft.getByText("Bozza completa, pronta per il salvataggio.")).toBeVisible()
  expect(consoleErrors).toEqual([])
})

test("quick parse playground shows missing fields without inventing data", async ({ page }) => {
  await page.goto("/admin/quick-parse")
  await page.getByLabel("Frase ricevuta").fill("domani sera 4 fuori vista mare")
  await page.getByRole("button", { name: "Interpreta frase" }).click()
